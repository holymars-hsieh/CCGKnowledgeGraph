...
內核運算機制：關於大腦內部的「雙核對抗 (GAN)」與「三網博弈 (DMN/TPN/SN)」機制，請參閱專門文件 [KG-CORE] 神經與算法的同構性。

宏觀社會拓撲：關於這些節點如何組網形成文明，請參閱 [KG-CORE] 社會拓撲：文明計算圖的宏觀架構。

神經參數差異：關於不同型號節點的參數差異，請參閱 [KG-CORE] 智人節點的超參數設定。